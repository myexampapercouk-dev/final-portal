...
// Shared sidebar + header shell (stitch "British Prep Specialist" design)
// used by the Parent and Admin portals. `navItems` is [{key,to,label,icon}].
export default function Shell({ active, navItems, roleLabel, children }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.surface, fontFamily: T.bodyFont }}>
      <header style={styles.header}>
        <Link to={navItems[0]?.to || '/'} style={styles.brandLink}>
        ...
        <div style={styles.headerRight}>
          <div style={{ display: 'flex', flexDirection: 'column', textAlign: 'right', lineHeight: 1.2 }}>
            ...
            <span style={{ fontSize: 11, color: T.onSurfaceVariant }}>{roleLabel}</span>
          </div>
      ...
      <aside style={styles.sidebar}>
        <div style={styles.sidebarTop}>
          ...
          <div>
            <div style={{ fontSize: 12.5, fontWeight: 700, color: T.onSurface }}>{roleLabel} Console</div>
